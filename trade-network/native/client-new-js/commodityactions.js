...
class CommodityActions {
    ...
    async createCommodity(tradingSymbol, description, mainExchange, quantity, owner) {
        ...
        const exists = await this.contract.evaluateTransaction('existsCommodity', tradingSymbol);
        ...
        const res = await this.contract.evaluateTransaction('getCommodity', tradingSymbol);
    ...
    async run() {
        ...
        let res = await this.contract.evaluateTransaction('getCommodity', 'TempCom');
        this.displayResource(res.toString('utf8'));
